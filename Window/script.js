...
const changeModalState = state => {
  (0,_modal__WEBPACK_IMPORTED_MODULE_0__.blockTrigger)('.popup_calc_button', 'block');
  (0,_modal__WEBPACK_IMPORTED_MODULE_0__.blockTrigger)('.popup_calc_profile_button', 'block');
  function addToState(elemSelector, eventType, propName) {
    let isCheck = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
        ...
        switch (elemSelector) {
          ...
          case '#height':
            ...
            if (state.width && state.height) {
              (0,_modal__WEBPACK_IMPORTED_MODULE_0__.blockTrigger)('.popup_calc_button', 'unblock');
            }
            break;
          case '#view_type':
            state[propName] = e.target.value;
          ...
          case '.checkbox':
            ...
            state[propName] = e.currentTarget.nextElementSibling.getAttribute('id');
            if (state.windowProfile) {
              (0,_modal__WEBPACK_IMPORTED_MODULE_0__.blockTrigger)('.popup_calc_profile_button', 'unblock');
            }
            break;
        }
      });
  ...
  ;
  addToState('.balcon_icons_img', 'click', 'windowType');
  addToState('#width', 'input', 'width', true);
  addToState('#height', 'input', 'height', true);
  addToState('#view_type', 'change', 'windowProfile');
  addToState('.checkbox', 'change', 'windowGlazing');
};
...
const forms = state => {
  ...
  const message = {
    ...
    success: "Дякуємо! Ми скоро з вам зв'яжемось!",
    failure: 'Щось пішло не за планом!(('
  };
  let phoneMask;
  inputs.forEach(input => input.addEventListener('input', () => {
    phoneMask = new imask__WEBPACK_IMPORTED_MODULE_0__["default"](input, {
  ...
  forms.forEach(form => {
    form.addEventListener('submit', e => {
      ...
      formData.set('user_phone', phoneMask.unmaskedValue);
      if (form.hasAttribute('data-calcform')) {
      ...
      postData('assets/server.php', formData).then(res => {
        ...
        setTimeout(() => {
          messageBlock.remove();
          if (form.hasAttribute('data-calcform')) {
            form.closest('.popup_calc_end').style.display = 'none';
          }
          (0,_modal__WEBPACK_IMPORTED_MODULE_1__.clearCalcData)();
        }, 3000);
      });
    });
  });
...
const gallery = () => {
  const workExamples = document.querySelector('.works'),
    popupWorks = document.createElement('div'),
    img = document.createElement('img'),
    scrollWidth = (0,_modal__WEBPACK_IMPORTED_MODULE_0__.getScrollWidth)();
  popupWorks.appendChild(img);
  popupWorks.classList.add('popup', 'animate__animated', 'animate__fadeIn');
  img.style.height = '75vh';
  popupWorks.style.justifyContent = 'center';
  popupWorks.style.alignItems = 'center';
  workExamples.append(popupWorks);
  workExamples.addEventListener('click', e => {
    e.preventDefault();
    if (e.target && e.target.matches('.preview')) {
      const url = e.target.parentNode.getAttribute('href');
      popupWorks.style.display = 'flex';
      document.body.style.marginRight = `${scrollWidth}px`;
      document.body.style.overflow = 'hidden';
      img.src = url;
    } else if (e.target && e.target == popupWorks) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "blockTrigger": () => (/* binding */ blockTrigger),
/* harmony export */   "clearCalcData": () => (/* binding */ clearCalcData),
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__),
/* harmony export */   "getScrollWidth": () => (/* binding */ getScrollWidth)
/* harmony export */ });
const modal = state => {
  function bindModal(triggerOpenModalSelector, modalSelector, closeSelector, timerOpenModal) {
    let closeClickOverlay = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 'true';
    const triggerElem = document.querySelectorAll(triggerOpenModalSelector),
      modal = document.querySelector(modalSelector),
      allModals = document.querySelectorAll('[data-modal]'),
      close = document.querySelector(closeSelector);
    triggerElem.forEach(el => {
    ...
    //     if(!e.target.matches('.popup_engineer .form')) {
    ...
    modal.addEventListener('click', e => {
      if (e.target == modal && closeClickOverlay || e.target.closest(closeSelector)) {
        allModals.forEach(item => {
          item.style.display = 'none';
          clearCalcData();
        });
  ...
  function openModal(elem) {
    let timerOpenModal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  function showModalByTime() {
    let timer = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 3000;
  ...
  ;
  const timerId = showModalByTime(60000);
  bindModal('.popup_engineer_btn', '.popup_engineer', '.popup_engineer .popup_close');
  bindModal('.phone_link', '.popup', '.popup_close', timerId);
  bindModal('.popup_calc_btn', '.popup_calc', '.popup_calc_close');
  bindModal('.popup_calc_button', '.popup_calc_profile', '.popup_calc_profile_close', null, false);
  bindModal('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close', null, false);
};
...
;
const blockTrigger = (elem, action) => {
  let trigger;
...
const clearCalcData = () => {
  const windowType = document.querySelectorAll('.balcon_icons_img'),
    ...
    windowGlazing = document.querySelectorAll('.checkbox');
  blockTrigger('.popup_calc_button', 'block');
  blockTrigger('.popup_calc_profile_button', 'block');
  windowType.forEach((item, i) => {
  ...
  width.value = '';
  height.value = '';
  windowProfile.value = 'tree';
  windowGlazing.forEach(chekbox => {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
const tabs = function (headerSelector, tabsSelector, contentSelector, activeTabClass) {
  ...
  function showTabContent() {
    ...
    content[i].style.overflowX = 'hidden';
...
const timer = (id, deadline) => {
  ...
  const setTimer = (selector, endTime) => {
    const timerBlock = document.querySelector(selector),
      timerInString = timerBlock.querySelector('#inlinetimer'),
      d = timerBlock.querySelector('#days'),
      h = timerBlock.querySelector('#hours'),
      m = timerBlock.querySelector('#minutes'),
      s = timerBlock.querySelector('#seconds'),
      intervalId = setInterval(updateTimer, 1000),
      t = new Date(endTime);
    updateTimer();
    const months = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];
    timerInString.textContent = t.getDate() + ` ${months[t.getMonth()]} ` + t.getFullYear() + ' года';
    function updateTimer() {
      ...
      if (leftTime <= 0) {
        ...
        d.textContent = days <= 9 ? '0' + days : days;
        h.textContent = hours <= 9 ? '0' + hours : hours;
        m.textContent = minutes <= 9 ? '0' + minutes : minutes;
        s.textContent = seconds <= 9 ? '0' + seconds : seconds;
      }
    }
  };
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div[data-decor]', 'after_click');
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.sale', '2023-04-01');
  (0,_modules_gallery__WEBPACK_IMPORTED_MODULE_6__["default"])();
});
})();
